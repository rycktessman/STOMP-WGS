#uses scripts from VCFMIX (https://github.com/davidhwyllie/VCFMIX)
...
parser.add_argument("-p", "--path", dest="VCFMIX_path")
parser.add_argument("-i", "--id", dest="seq_id")
parser.add_argument("-f", "--file", dest="seq_file")
args=parser.parse_args()
print(args)
print(args.VCFMIX_path)
print(args.seq_file)

VCFMIX_path=args.VCFMIX_path
sys.path.insert(1, VCFMIX_path)
...
class BinomialTest():
	...
	def compute(self, minor_variant_count, depth):
		...
		try:
			...
			# not precomputed
			self.p_values[key] = stats.binomtest(k=minor_variant_count,n=depth,p=self.expectedErrorRate).pvalue   # do the test if any variation
...
class lineageScan(vcfScan):
	...
	def __init__(self,
				 expectedErrorRate = 0.001,
				 lineage_definition_file=os.path.join(VCFMIX_path,"refdata", "Coll2014_LinSpeSNPs_final.csv"),
				 exclusion_position_file = os.path.join(VCFMIX_path,"refdata", "exclusion_nt.txt"),
				 infotag = 'BaseCounts'
				 ):
		""" creates a vcfScan object.
		Inputs:
...
# scan an input file
inputfile=seq_file + ".counts.vcf"
print(inputfile)
res = v.parse(vcffile = inputfile, guid=seq_id)

# print details of the regions scanned
print(v.region_stats)

# export details of the regions scanned
outputfile=seq_file + ".vcfmix.csv"
v.region_stats.to_csv(outputfile)

# compute F2 and F47 statistics
summary1 = v.f_statistics()
print(summary1)

# export F2 and F47 statistics
outputfile=seq_file + ".vcfmetrics.csv"

summary1_df = pd.DataFrame(summary1, index=[0])
summary1_df.insert(loc=0, column='id', value=seq_id)
print(summary1_df)
summary1_df.to_csv(outputfile)
